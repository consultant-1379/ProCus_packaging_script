...
Note: 	It is presumed that the Customer Site has a working Ericsson Network IQ Server(*), 
      	...
      	* Revision of installed SW should be one of the following: AOM901163 R1Z
...
Note :	Prior to installation, take snapshots of the system.
		Steps to take snapshots on Rack/Single Blade/Multi Blade deployments are available in Ericsson Network IQ Statistics System Administration Guide (4/1543-CSA 113 63/1)

		After taking snapshot of the system execute the following steps to install the Tech Pack.

	*(In case of MultiBlade, Run this below steps on Coordinator blade unless specified)*
...
1.3 Restart all the ENIQ services

	In case of MultiBlade, Run this step on Coordinator
	# cd /eniq/admin/bin
	# /usr/bin/bash ./manage_deployment_services.bsh -a stop -s ALL
	
	Once all the services have been disabled on all 4 blades we can start it as :
	
	Steps mentioned below has to be followed on Coordinator
	# cd /eniq/admin/bin
	# /usr/bin/bash ./manage_deployment_services.bsh -a start -s ALL 
	# /usr/bin/su - dcuser -c "engine -e changeProfile Normal"

1.4	Check that services are online on each blade in the deployment:
    • ENIQ Statistics Coordinator Server
    • ENIQ Statistics Engine Server
	• ENIQ Statistics Reader Server(s)
	
	# /usr/bin/svcs -a | grep eniq

1.5	Change user permisson for packagelist.txt and update_feature_list.sh
	# chmod 777 /var/tmp/ENIQ_PC_upgrade/update_feature_list.sh
	# chmod 777 /var/tmp/ENIQ_PC_upgrade/packagelist.txt

	
1.6	Switch user to dcuser for installation steps below.
	# su - dcuser
...
4.2	Restart all the ENIQ services and set profile to Normal
	...
	Steps mentioned below has to be followed on Coordinator blade
	# cd /eniq/admin/bin
	# /usr/bin/bash ./manage_deployment_services.bsh -a start -s ALL
...
4.3	Enable the ENIQ rolling snapshot service 
	For MultiBlade, Below steps has to be followed on all the four blades in sequence [ Coordinator, Engine , Reader_1, Reader_2]. 	
 	# /usr/sbin/svcadm enable svc:/eniq/roll-snap
...
7.2.1	Delete ZFS Snapshots
	...
	# cd /eniq/bkup_sw/bin
	# bash ./manage_zfs_snapshots.bsh -a delete -f ALL=<snap_label>
